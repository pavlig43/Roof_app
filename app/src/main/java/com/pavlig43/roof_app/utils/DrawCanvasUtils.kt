...
fun rulerOnCanvasPDF(
    ...
    if (!horizontal) {
        ...
        canvas.drawLine(
            ...
            paddingWidth, countCeilMetersWidth * oneCMInHeightYtPx * 100 + paddingHeight,
            paint
        )
 ...
 */
 fun drawSheet(
    ...
    val lenOfSheetInPX = paintText.measureText("$lenOfSheet cm")// длина текста в пикселях , используется, чтобы середина текста находилать посередине листа, без этого параметра сам текст начинается в середине листа, а  оканчивается за пределами листа
