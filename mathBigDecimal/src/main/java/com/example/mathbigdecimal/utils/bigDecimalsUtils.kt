...
fun BigDecimal.sqrt1(mathContext: MathContext): BigDecimal {
    ...
    do {
        previousGuess = guess
        guess = this.divide(guess, mathContext) // Делим число на текущую оценку
            .add(guess)                         // Суммируем с текущей оценкой
            .divide(two, mathContext)           // Делим пополам
    } while (guess.subtract(previousGuess).abs() > BigDecimal.ZERO) // Пока разница не станет незначительной

    return guess
}

fun main() {
    println(BigDecimal(2).sqrt1(MathContext(10)))
}